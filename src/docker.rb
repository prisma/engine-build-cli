...
class DockerCommands
  ...
  def self.run_rust_tests(context)
    ...
    Command.new("docker-compose", *compose_flags, "up", "-d", "test-db-postgres", "test-db-mysql57", "test-db-mysql8").puts!.run!.raise!
  ...
  def self.run_connector_test_kit(context, connector)
    compose_flags = ["--file", "#{context.server_root_path}/.buildkite/engine-build-cli/docker-test-setups/docker-compose.test.all.yml"]
    Command.new("docker-compose", *compose_flags, "up", "-d", "test-db-postgres", "test-db-mysql57").puts!.run!.raise!
